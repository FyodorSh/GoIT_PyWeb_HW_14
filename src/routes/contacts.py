...
@router.get("/birthday/", response_model=List[ContactBirthday], dependencies=[Depends(RateLimiter(times=20,
                                                                                                  seconds=60))])
...
async def get_contact(contact_id: int = Path(0, ge=0), db: Session = Depends(get_db),
